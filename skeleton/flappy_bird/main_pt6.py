...
def main():
    ...
    while run:
        ...
        score_text = None  # TODO use font.render() to make a score text
        ...
        if pygame.sprite.spritecollide(bird.sprite, ground, False):
        ...
        if pipe_timer <= 0 and bird.sprite.control:
            ...
            bp_y = toppipe_y + assets.bird_up.get_height() + randint(15, 200) # TODO: fix this
            bp_y = min(bp_y, y_ground - 50)
            pipe_x = WIDTH
            pipes.add(Pipe(pipe_x, toppipe_y, assets.pipe_top, SCROLL_SPEED, "top"))
            pipes.add(Pipe(pipe_x, bp_y, assets.pipe_bottom, SCROLL_SPEED, "bottom"))
