...
class FloodFillMMS:
    ...
    def in_bounds(self, coord: tuple, mat: Literal['dist', 'hwalls', 'vwalls']='dist') -> bool:
        coord = np.array(coord)
        if mat == 'dist':
            return np.all((coord >= 0)) and np.all((coord < self.size))
        elif mat == 'hwalls':
            return np.all((coord >= 0)) and (coord[0] < self.size - 1) and (coord[1] < self.size)
        else:
            return np.all((coord >= 0)) and (coord[0] < self.size) and (coord[1] < self.size - 1)
    ...
    def update_walls(self, current_location: tuple, ort: Orientation) -> None:
        ...
        if ort.disp()=="N":
            walls['left'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]-1) if self.in_bounds((pos[0], pos[1]-1), 'vwalls') else None} # note that we check if the wall is in bounds, TODO: double check in bounds part
            walls['right'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'vwalls') else None}
            walls['front'] = {'wall': self.hwalls, 'coord': (pos[0]-1, pos[1]) if self.in_bounds((pos[0]-1, pos[1]), 'hwalls') else None}
            walls['back'] = {'wall': self.hwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'hwalls') else None}
            dir_to_heading = {
                'left': 'w', 
                'right': 'e',
                'front': 'n',
            ...
            walls['left'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'vwalls') else None}
            walls['right'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]-1) if self.in_bounds((pos[0], pos[1]-1), 'vwalls') else None}
            walls['front'] = {'wall': self.hwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'hwalls') else None}
            walls['back'] = {'wall': self.hwalls, 'coord': (pos[0]-1, pos[1]) if self.in_bounds((pos[0]-1, pos[1]), 'hwalls') else None}
            dir_to_heading = {
                'left': 'e', 
                'right': 'w',
                'front': 's',
            ...
            walls['back'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]-1) if self.in_bounds((pos[0], pos[1]-1), 'vwalls') else None} #
            walls['front'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'vwalls') else None}
            walls['left'] = {'wall': self.hwalls, 'coord': (pos[0]-1, pos[1]) if self.in_bounds((pos[0]-1, pos[1]), 'hwalls') else None}
            walls['right'] = {'wall': self.hwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'hwalls') else None}
            dir_to_heading = {
                'left': 'n', 
                'right': 's',
                'front': 'e',
            ...
            walls['front'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]-1) if self.in_bounds((pos[0], pos[1]-1), 'vwalls') else None}
            walls['back'] = {'wall': self.vwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'vwalls') else None}
            walls['right'] = {'wall': self.hwalls, 'coord': (pos[0]-1, pos[1]) if self.in_bounds((pos[0]-1, pos[1]), 'hwalls') else None}
            walls['left'] = {'wall': self.hwalls, 'coord': (pos[0], pos[1]) if self.in_bounds((pos[0], pos[1]), 'hwalls') else None}
            dir_to_heading = {
                'left': 's', 
                'right': 'n',
                'front': 'w',
        ...
        for w, val in zip(sorted(walls.keys()), [API.wallBack, API.wallFront, API.wallLeft, API.wallRight]):
    ...
    def prep_move(self, current_location: tuple, ort: Orientation, mode: Literal["normal", "return"]) -> None:
        pos = mmspos_to_mat(current_location, self.size)
        # Steps: 
        ...
        while redo:
            ...
            current_distance = None # TODO: fill me in
            n_dists = []
            n_heads = []
            for h, n in zip(headings, neighbors):
                if self.in_bounds(n):
                    neighb_dist = None # TODO: fill me in with the neighbor's distanct (n)
